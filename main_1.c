...
struct card//校园卡结构体
{
    ...
    struct Money wash_ticket;    //洗漱票余额
...
struct Money moneySub(struct Money m1 , struct Money m2){
    struct Money r;
    r.yuan = r.jiao = r.fen = 0;
    if(m1.fen < m2.fen){
        m1.jiao -= 1;
        m1.fen += 10;
    }
    ...
    r.jiao = m1.jiao - m2.jiao;
    r.yuan = m1.yuan - m2.yuan;
    return r;
}
struct Money moneyAdd(struct Money m1 , struct Money m2){
    struct Money r;
    if(m1.fen + m2.fen >= 10){
        m1.fen -= 10;
        r.jiao += 1;
    ...
    r.fen = m1.fen + m2.fen;
    if(m1.jiao + m2.jiao >= 10){
        m1.jiao -= 10;
        r.yuan += 1;
    }
    r.jiao = r.jiao + m1.jiao + m2.jiao;
    r.yuan = r.yuan + m1.yuan + m2.yuan;
    return r;
}
...
struct card * AddMoney(struct card * cardHead){
    ...
    if(p -> number != n)printf("Number not exists.");
        ...
        p -> balance = moneyAdd((p -> balance),m);
...
void Modify(struct card * cardHead){
    ...
    if(p -> listHead -> next == NULL) printf("Record list is empty.");
        ...
        if((q -> time.year != mt.year) || (q -> time.month != mt.month) || (q -> time.day != mt.day) || (q -> time.hour != mt.hour) || (q -> time.minute != mt.minute) || (q -> time.second != mt.second)) printf("No such record.");
            ...
            if(q -> type == 1){
                p -> balance = moneyAdd((p -> balance),(q -> price));
            }
            else{
                p -> balance =moneySub((p -> balance), (q -> price));
            }
...
void Bathroom(struct card * cardHead){
    ...
    if(p -> number != n) printf("Number not exists.");
        ...
        if((n / 100) != 10){
            ...
            struct Money m;
            m.yuan = (int)w;
            m.jiao = (int)((w - (r -> price.yuan)) * 10);
            m.fen = (int)((w - (r -> price.yuan) - (r -> price.jiao)) * 100);
            p -> wash_ticket = moneySub((p -> wash_ticket),m);
            if(p -> wash_ticket.yuan <= 0){
                printf("Wash_ticket is empty.");
                p -> wash_ticket = moneyAdd(p -> wash_ticket, m);
                p -> balance = moneySub((p -> balance),moneySub(m,(p -> wash_ticket)));
                p -> wash_ticket.yuan = 0;
                p -> wash_ticket.jiao = 0;
                p -> wash_ticket.fen = 0;
            }
        }
    }
...
//    print(cardHead);
    struct Money m1,m2;
    m1.yuan = 0;m1.jiao = 0;m1.fen = 1;
    m2.yuan = 8;m2.jiao = 9;m2.fen = 9;
    struct Money r = moneySub(m1,m2);
    printf("%d元%d角%d分\n",r.yuan,r.jiao,r.fen);
